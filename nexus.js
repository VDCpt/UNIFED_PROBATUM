   ...
   const handler = {
    apply: function(target, thisArg, argumentsList) {
        ...
        if (typeof url === 'string') {
            ...
            if (window.ForensicLogger) window.ForensicLogger.addEntry('NETWORK_CALL', { url });
        }
        ...
        return Reflect.apply(target, thisArg, argumentsList)
            .catch(err => {
                ...
                if (typeof reqUrl === 'string' && reqUrl.includes('api.unifed.com')) {
                    if (window.ForensicLogger) window.ForensicLogger.addEntry('NETWORK_FAILURE_SILENT', { url: reqUrl, error: err.message });
                    throw err;
                }
                console.warn(`[NEXUS-AUDIT] Falha de comunicação externa: ${reqUrl} | Motivo: ${err.message}`);
                if (window.ForensicLogger) window.ForensicLogger.addEntry('NETWORK_FAILURE', { url: reqUrl, error: err.message });
                throw err;
            });
    }
};
...
(function _nexusForecastATF() {
    ...
    function _injectRiscoFuturoPanel(forecast) {
        ...
        headerDiv.innerHTML = '<div style="color:#A855F7;font-size:0.9rem;font-weight:bold;letter-spacing:0.06em">🔮 MOTOR PREDITIVO NEXUS ATF · RISCO FUTURO (6 MESES)</div>' +
            ...
            '<table style="width:100%;border-collapse:collapse;font-size:0.7rem;color:rgba(255,255,255,0.8)">' +
                ...
                '<tbody>' +
                    forecast.labels.map(function(lbl, i) {
                        ...
                        return '<tr>' +
                            '<td style="border:1px solid rgba(168,85,247,0.15);padding:5px 10px;color:#A855F7">' + escapeHtml(lbl) + '</td>' +
        ...
        footerDiv.innerHTML = '<strong style="color:rgba(168,85,247,0.8)">⚙ ' + _T('Metodologia Preditiva (NEXUS ATF):', 'Predictive Methodology (NEXUS ATF):') + '</strong> ' +
...
(function _nexusBlockchainExplorer() {
    ...
    async function _openBlockchainExplorerModal() {
        ...
        header.innerHTML = '<div><div style="color:#00E5FF;font-size:0.85rem;font-weight:700;letter-spacing:0.08em">⛓️ BLOCKCHAIN EVIDENCE EXPLORER · NEXUS v13.12.0-PURE</div>' +
            '<div style="color:rgba(255,255,255,0.4);font-size:0.62rem;margin-top:2px">' + _T('SHA-256 Individual · OTS Status · Cadeia de Custódia · ', 'SHA-256 Individual · OTS Status · Chain of Custody · ') +
            enriched.length + ' ' + _T('documento', 'document') + (enriched.length !== 1 ? 's' : '') + '</div></div>' +
            '<button id="nexusExplorerCloseBtn" style="background:transparent;border:1px solid rgba(0,229,255,0.3);color:#00E5FF;cursor:pointer;padding:5px 14px;font-family:inherit;font-size:0.72rem;letter-spacing:1px;border-radius:3px;transition:background 0.2s;" onmouseover="this.style.background=\'rgba(0,229,255,0.1)\'" onmouseout="this.style.background=\'transparent\'">✕ FECHAR</button>';
        ...
        legend.innerHTML = '<span>🔗 <span style="color:#4ADE80">ANCORADO</span> — Hash registado na cadeia de custódia PROBATUM (Nível 1 ativo)</span>' +
        ...
        if (enriched.length === 0) {
            contentDiv.innerHTML = '<div style="text-align:center;padding:32px;color:rgba(255,255,255,0.3);font-size:0.8rem">' +
                '📭 ' + _T('Nenhum documento registado na sessão atual.<br>', 'No documents registered in the current session.<br>') +
                '<span style="font-size:0.65rem">' + _T('Carregue evidências para ativar o Explorer.', 'Upload evidence to activate the Explorer.') + '</span></div>';
        } else {
                    ...
                    '<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px;flex-wrap:wrap">' +
                        '<div style="display:flex;align-items:center;gap:8px">' +
                            ...
                            '<div>' +
                                ...
                                '<div style="color:rgba(255,255,255,0.4);font-size:0.62rem">' + escapeHtml(item.type) + '</div>' +
                            '</div>' +
                        '</div>' +
        ...
        footer.innerHTML = '⚙ ' + _T('NEXUS Blockchain Explorer · SHA-256 independente por ficheiro · ', 'NEXUS Blockchain Explorer · Individual SHA-256 per file · ') +
            'Art. 125.o CPP · ISO/IEC 27037:2012 · DORA (UE) 2022/2554 · Read-Only sobre UNIFEDSystem · ' + new Date().toLocaleString('pt-PT');

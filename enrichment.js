...
(function _installUNIFEDUtils() {
	...
	if(typeof window.UNIFEDSystem.utils === 'undefined') {
	...
	if(typeof _utils.formatCurrency !== 'function') {
		_utils.formatCurrency = function _uFormatCurrency(val) {
			if(typeof window.formatCurrency === 'function' && window.formatCurrency !== _utils.formatCurrency) {
...
function _buildForensicContext(analysis) {
	...
	if(c.discrepanciaCritica && Math.abs(c.discrepanciaCritica) > 0) lines.push('OMISSAO DE CUSTOS: ' + _fmtEur(c.discrepanciaCritica) + ' (' + (c.percentagemOmissao || 0).toFixed(2) + '%)');
	if(c.discrepanciaSaftVsDac7 && Math.abs(c.discrepanciaSaftVsDac7) > 0) lines.push('OMISSAO RECEITA SAF-T vs DAC7: ' + _fmtEur(c.discrepanciaSaftVsDac7));
	...
	if(v.level && v.level.pt) {
		lines.push('---');
		lines.push('VEREDICTO: ' + v.level.pt + ' -- ' + (v.percent || 'N/A'));
	}
	return lines.filter(function(l) {
		return l !== '---';
	}).length === 0 ? '[DADOS INSUFICIENTES]' : lines.join('\n');
}

function _buildLegalContext() {
	return Object.keys(LEGAL_KB).map(function(code) {
		var arts = LEGAL_KB[code];
		return code + ':\n' + Object.values(arts).map(function(a) {
			return '  . ' + a;
		}).join('\n');
	}).join('\n\n');
}
...
function _fallbackNarrative(reason) {
	return ['SINTESE JURIDICA - MODO DE SEGURANCA FORENSE', '[Nota: IA indisponivel - ' + reason + ']', '', 'Seccao A - QUALIFICACAO JURIDICA DOS FACTOS', 'As discrepancias apuradas pelo motor UNIFED-PROBATUM constituem indicios de omissao tributaria ' + 'nos termos dos artigos 103.o e 104.o do RGIT. A divergencia entre os valores reportados pela plataforma ' + '(DAC7) e os valores declarados pelo sujeito passivo configura, prima facie, o elemento objetivo do tipo ' + 'de ilicito de fraude fiscal qualificada, por envolver vantagem patrimonial ilegitima superior ao limiar ' + 'legalmente previsto de 15.000 EUR.', '', 'Seccao B - ENQUADRAMENTO LEGAL E TRIBUTARIO', 'A omissao de base tributavel implica a obrigacao de regularizacao nos termos do Art. 78.o do CIVA. ' + 'O IVA em falta, calculado as taxas de 23% e 6%, acresce ao imposto em falta ao abrigo do Art. 2.o ' + 'do CIVA (autoliquidacao). O lucro tributavel devera ser corrigido ao abrigo do Art. 17.o do CIRC, ' + 'com incidencia de tributacao autonoma sobre encargos nao documentados (Art. 88.o CIRC).', '', 'Seccao C - CONCLUSOES DE ADMISSIBILIDADE', '(I) CONSOLIDACAO DA PROVA MATERIAL: A densidade tecnica da evidencia digital extraida via UNIFED PURE e suficiente para a demonstracao do nexo de causalidade na subdeclaracao de base tributavel. A metodologia forense aplicada (ISO/IEC 27037:2012 · Art. 125.o CPP) e replicavel e verificavel por perito independente.', '', '(II) IMPACTO SISTEMICO ESTIMADO (SIMULACAO): Com base na metrica de discrepancia apurada neste processo e extrapolando para a estabilidade do setor (38.000 condutores — IMT), estima-se um passivo fiscal oculto sistemico de magnitude superior a 500M EUR. Esta projecao carece de prova pericial complementar em sede de execucao e nao constitui prova de facto neste processo concreto.', '', '(III) RECOMENDACAO PROCESSUAL: Face a robustez dos artefactos apresentados, recomenda-se a inversao do onus da prova (Art. 344.o CC e Art. 100.o CPPT), mantendo-se a disponibilidade para o pleno exercicio do contraditorio (Art. 327.o CPP) mediante acesso aos logs brutos de calculo de comissoes, sob custodia exclusiva da plataforma. O UNIFED-PROBATUM realiza RECONSTITUICAO DA VERDADE MATERIAL DIGITAL — nao contabilidade — distinção juridicamente relevante para a admissibilidade da prova pericial.', '', 'Seccao D - ESTRATEGIA DE CONTRA-INTERROGATORIO', 'Argumento da Defesa: "Os valores reportados pelo DAC7 incluem taxas de cancelamento e reembolsos ' + 'que nao constituem rendimento tributavel do prestador."', 'Resposta Pericial: Nos termos do Art. 36.o do CIVA, cada componente da remuneracao deve constar ' + 'de fatura discriminada. A ausencia de faturacao discriminada por componente confirma a omissao.', '', 'Argumento da Defesa: "A discrepancia resulta de diferencas de cambio e ajustamentos de plataforma ' + 'comunicados tardiamente."', 'Resposta Pericial: O Art. 29.o do CIVA impoe emissao no prazo de 5 dias uteis. Ajustamentos ' + 'tardios nao afastam a obrigacao declarativa do periodo original (Art. 78.o CIVA).', '', 'Argumento da Defesa: "O contribuinte nao tinha conhecimento tecnico das obrigacoes DAC7."', 'Resposta Pericial: O regime DAC7 esta em vigor em Portugal desde 1 de janeiro de 2023 ' + '(Lei n.o 17/2023) e a plataforma tem obrigacao de informar o prestador nos termos do Art. 8.o ' + 'da Diretiva. A ignorancia da lei nao aproveita (Art. 6.o CC).'].join('\n');
}
// ============================================================================
...
async function renderSankeyToImage(analysis) {
	...
	[{
		color: '#3B82F6',
		text: 'Volume Real (Ganhos Extrato): ' + _fmtEur(volumeReal)
	}, {
		color: '#10B981',
		text: 'Faturado (SAF-T): ' + _fmtEur(faturadoSaft)
	}, {
		color: '#6366F1',
		text: 'Reportado (DAC7): ' + _fmtEur(reportadoDac7)
	}, {
		color: '#EF4444',
		text: 'Omissao/Retencao Sistematica: ' + _fmtEur(omissaoRet) + ' (' + (c.percentagemOmissao || 0).toFixed(2) + '%)'
...
function generateIntegritySeal(masterHash, doc, x, y, sealSize) {
	...
	var R2 = SZ * 0.28;
	var bytes = [];
	for(var i = 0; i < Math.min(masterHash.length, 64); i += 2) bytes.push(parseInt(masterHash.substring(i, i + 2), 16));
	...
	doc.text('PROBATUM INTEGRITY SEAL', CX, y + 3.5, {
		align: 'center'
	});
	doc.text('v13.12.0-PURE \u00b7 SHA-256', CX, y + 6.5, {
		align: 'center'
	});
	...
	doc.text(masterHash.substring(0, 16) + '...', CX, y + SZ - 3, {
		align: 'center'
	});
...
async function exportDOCX(xmlInject) {
	...
	var para = function(text, bold, size, color, align) {
		...
		color = color || '000000';
		align = align || 'left';
		return '<w:p><w:pPr><w:jc w:val="' + align + '"/><w:spacing w:after="120"/></w:pPr><w:r>' + '<w:rPr><w:sz w:val="' + size + '"/><w:szCs w:val="' + size + '"/>' + (bold ? '<w:b/><w:bCs/>' : '') + '<w:color w:val="' + color + '"/></w:rPr>' + '<w:t xml:space="preserve">' + xe(text) + '</w:t></w:r></w:p>';
	};
	var tc = function(text, bold, w, shade) {
		bold = bold || false;
		w = w || 4000;
		return '<w:tc><w:tcPr><w:tcW w:w="' + w + '" w:type="dxa"/>' + (shade ? '<w:shd w:val="clear" w:color="auto" w:fill="' + shade + '"/>' : '') + '<w:tcBorders><w:top w:val="single" w:sz="4" w:color="AAAAAA"/><w:left w:val="single" w:sz="4" w:color="AAAAAA"/><w:bottom w:val="single" w:sz="4" w:color="AAAAAA"/><w:right w:val="single" w:sz="4" w:color="AAAAAA"/></w:tcBorders>' + '</w:tcPr><w:p><w:pPr><w:spacing w:after="60"/></w:pPr><w:r><w:rPr><w:sz w:val="18"/><w:szCs w:val="18"/>' + (bold ? '<w:b/><w:bCs/>' : '') + '</w:rPr><w:t xml:space="preserve">' + xe(text) + '</w:t></w:r></w:p></w:tc>';
	};
	var tr = function(cells) {
		return '<w:tr>' + cells.join('') + '</w:tr>';
	};
	var tbl = function(rows) {
		return '<w:tbl><w:tblPr><w:tblW w:w="9000" w:type="dxa"/>' + '<w:tblBorders><w:insideH w:val="single" w:sz="4" w:color="DDDDDD"/>' + '<w:insideV w:val="single" w:sz="4" w:color="DDDDDD"/></w:tblBorders></w:tblPr>' + rows.join('') + '</w:tbl>';
	};
	var hr = function() {
	...
	try {
		if(typeof generateLegalNarrative === 'function') aiNarrative = await generateLegalNarrative(sys.analysis);
	} catch (_e) {}
	...
	var narrativeParas = aiNarrative.split('\n').map(function(l) {
		...
		return l.replace(_ghostRe, '').trim();
	}).filter(function(l) {
		return l.length > 0;
	}).map(function(l) {
	...
	try {
		...
		if(typeof ForensicLogger !== 'undefined') ForensicLogger.addEntry('DOCX_EXPORT_COMPLETED', {
...
function computeTemporalAnalysis(monthlyData, analysis) {
	...
	if(months.length === 0) {
		return {
			...
			outlierMonths: [],
			persistenceScore: 0,
			persistenceLabel: (typeof window.currentLang !== 'undefined' && window.currentLang === 'en') ? 'Insufficient data. Upload monthly statements (filename must include YYYYMM).' : 'Dados insuficientes. Carregue extratos mensais (nome AAAAMM).',
			trend: 'neutral',
			opportunisticPattern: false
		};
	}
	var ganhosSeries = months.map(function(m) {
		return (monthlyData[m].ganhos || 0);
	});
	var despesasSeries = months.map(function(m) {
		return (monthlyData[m].despesas || 0);
	});
	var discrepancySeries = months.map(function(m, i) {
		return Math.abs(despesasSeries[i] - ganhosSeries[i]);
	});
	var n = discrepancySeries.length;
	var mean = discrepancySeries.reduce(function(a, v) {
		return a + v;
	}, 0) / n;
	var variance = discrepancySeries.reduce(function(a, v) {
		return a + Math.pow(v - mean, 2);
	}, 0) / n;
	var stdDev = Math.sqrt(variance);
	var outlierMonths = months.filter(function(m, i) {
		return discrepancySeries[i] > mean + 2 * stdDev;
	});
	var sx = 0,
		sy = 0,
		sxy = 0,
		sx2 = 0;
	discrepancySeries.forEach(function(v, i) {
		sx += i;
		sy += v;
		sxy += i * v;
		sx2 += i * i;
	...
	var _atfT = function(pt, en) {
		return _atfLang === 'en' ? en : pt;
	};
	var persistenceLabel;
	if(persistenceScore >= 80) {
		persistenceLabel = _atfT('PADRAO DE OMISSAO SISTEMATICA DETETADO - Comportamento consistente com dolo para efeitos Art. 104.o RGIT.', 'SYSTEMATIC OMISSION PATTERN DETECTED - Behaviour consistent with intent under Art. 104 RGIT.');
	} else if(persistenceScore >= 55) {
		persistenceLabel = _atfT('PADRAO DE OMISSAO RECORRENTE - Indicios de negligencia grave ou comportamento oportunistico.', 'RECURRENT OMISSION PATTERN - Indications of gross negligence or opportunistic behaviour.');
	} else if(persistenceScore >= 30) {
		persistenceLabel = _atfT('OMISSOES PONTUAIS IDENTIFICADAS - Analise complementar recomendada.', 'OCCASIONAL OMISSIONS IDENTIFIED - Supplementary analysis recommended.');
	} else {
		persistenceLabel = _atfT('Omissoes esporadicas - possivel erro operacional.', 'Sporadic omissions - possible operational error.');
	}
	if(opportunisticPattern) {
		persistenceLabel += ' ' + _atfT('PADRAO OPORTUNISTICO: omissoes concentradas em meses de maior faturacao.', 'OPPORTUNISTIC PATTERN: omissions concentrated in highest-revenue months.');
	}
	return {
		months: months,
		ganhosSeries: ganhosSeries,
		despesasSeries: despesasSeries,
		discrepancySeries: discrepancySeries,
		mean: mean,
		stdDev: stdDev,
		...
		opportunisticPattern: opportunisticPattern
	};
}
...
async function generateTemporalChartImage(monthlyData, analysis) {
	...
	var cW = W - padL - padR,
		cH = H - padT - padB;
	ctx3.strokeStyle = 'rgba(0,229,255,0.3)';
	ctx3.lineWidth = 1;
	ctx3.beginPath();
	ctx3.moveTo(padL, padT);
	ctx3.lineTo(padL, padT + cH);
	...
	ctx3.beginPath();
	ctx3.moveTo(padL, mY);
	ctx3.lineTo(padL + cW, mY);
	ctx3.stroke();
	ctx3.setLineDash([]);
	if(atf.stdDev > 0) {
		var sigY = toY(atf.mean + 2 * atf.stdDev);
		ctx3.strokeStyle = 'rgba(239,68,68,0.3)';
		ctx3.setLineDash([3, 3]);
		ctx3.beginPath();
		ctx3.moveTo(padL, sigY);
		ctx3.lineTo(padL + cW, sigY);
		ctx3.stroke();
		ctx3.setLineDash([]);
		ctx3.fillStyle = 'rgba(239,68,68,0.7)';
		ctx3.font = '10px Courier New, monospace';
		ctx3.textAlign = 'left';
		ctx3.fillText('2\u03c3', padL + cW + 4, sigY + 4);
	}
	ctx3.strokeStyle = '#3B82F6';
	ctx3.lineWidth = 2;
	ctx3.beginPath();
	atf.ganhosSeries.forEach(function(v, i) {
		if(i === 0) ctx3.moveTo(toX(i), toY(v));
		else ctx3.lineTo(toX(i), toY(v));
	});
	ctx3.stroke();
	...
	[{
		...
		ctx3.font = '11px Courier New, monospace';
		ctx3.textAlign = 'left';
...
function openATFModal() {
	var sys = window.UNIFEDSystem;
	if(!sys) {
		console.warn('[UNIFED-ATF] UNIFEDSystem nao disponivel.');
		return;
	}
	...
	titleDiv.innerHTML = '<div style="color:#00E5FF;font-size:1.1rem;font-weight:bold;letter-spacing:0.08em">' + _T('⏳ ANÁLISE TEMPORAL FORENSE (ATF)', '⏳ FORENSIC TEMPORAL ANALYSIS (ATF)') + ' · v13.12.0-PURE</div>' + '<div style="color:rgba(255,255,255,0.5);font-size:0.72rem;margin-top:4px">' + _T('Tendências · Outliers 2σ · Índice de Recidiva Algorítmica · Read-Only', 'Trends · Outliers 2σ · Algorithmic Recidivism Index · Read-Only') + '</div>';
	...
	spDiv.innerHTML = '<div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap">' + '<div style="text-align:center;min-width:80px"><div style="font-size:2.4rem;font-weight:900;color:' + spColor + '">' + atf.persistenceScore + '</div><div style="color:rgba(255,255,255,0.5);font-size:0.65rem">/100 — SP</div></div>' + '<div style="flex:1"><div style="color:' + spColor + ';font-weight:bold;font-size:0.9rem;margin-bottom:4px">' + _T('SCORE DE PERSISTÊNCIA (SP)', 'PERSISTENCE SCORE (SP)') + '</div>' + '<div style="color:rgba(255,255,255,0.75);font-size:0.8rem;line-height:1.5">' + escapeHtml(atf.persistenceLabel) + '</div></div></div>' + '<div style="margin-top:12px;background:rgba(255,255,255,0.1);border-radius:4px;height:8px;overflow:hidden"><div style="height:100%;width:' + atf.persistenceScore + '%;background:' + spColor + ';border-radius:4px"></div></div>';
	...
	statsGrid.innerHTML = '<div style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.4);border-radius:6px;padding:12px;text-align:center">' + '<div style="color:rgba(255,255,255,0.5);font-size:0.65rem;margin-bottom:4px">' + _T('MESES ANALISADOS', 'MONTHS ANALYSED') + '</div>' + '<div style="color:#3B82F6;font-size:1.5rem;font-weight:bold">' + months.length + '</div></div>' + '<div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.4);border-radius:6px;padding:12px;text-align:center">' + '<div style="color:rgba(255,255,255,0.5);font-size:0.65rem;margin-bottom:4px">OUTLIERS &gt; 2\u03c3</div>' + '<div style="color:#EF4444;font-size:1.5rem;font-weight:bold">' + atf.outlierMonths.length + '</div></div>' + '<div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.4);border-radius:6px;padding:12px;text-align:center">' + '<div style="color:rgba(255,255,255,0.5);font-size:0.65rem;margin-bottom:4px">' + _T('TENDÊNCIA', 'TREND') + '</div>' + '<div style="color:#F59E0B;font-size:1rem;font-weight:bold;margin-top:6px">' + (atf.trend === 'ascending' ? '📈 ' + _T('ASCENDENTE', 'ASCENDING') : atf.trend === 'descending' ? '📉 ' + _T('DESCENDENTE', 'DESCENDING') : '➡️ ' + _T('ESTÁVEL', 'STABLE')) + '</div></div>' + '<div style="background:rgba(' + (atf.opportunisticPattern ? '239,68,68' : '16,185,129') + ',0.1);border:1px solid rgba(' + (atf.opportunisticPattern ? '239,68,68' : '16,185,129') + ',0.4);border-radius:6px;padding:12px;text-align:center">' + '<div style="color:rgba(255,255,255,0.5);font-size:0.65rem;margin-bottom:4px">' + _T('PADRÃO OPORTUNÍSTICO', 'OPPORTUNISTIC PATTERN') + '</div>' + '<div style="color:' + (atf.opportunisticPattern ? '#EF4444' : '#10B981') + ';font-size:0.9rem;font-weight:bold;margin-top:6px">' + (atf.opportunisticPattern ? '⚠ ' + _T('DETETADO', 'DETECTED') : '✓ ' + _T('NÃO DETETADO', 'NOT DETECTED')) + '</div></div>';
	...
	if(months.length === 0) {
		chartSection.innerHTML += '<div style="padding:2rem;text-align:center;background:rgba(0,0,0,0.5);border-radius:8px;margin-top:1rem;">' + '<i class="fas fa-chart-line" style="font-size:2rem;color:var(--warn-secondary);"></i>' + '<h4>' + _T('Análise Temporal Forense não disponível', 'Forensic Temporal Analysis not available') + '</h4>' + '<p>' + _T('O lote global não possui decomposição mensal. Os dados são processados em modo agregado.', 'The global batch does not have monthly breakdown. Data is processed in aggregate mode.') + '</p>' + '<p>' + _T('As discrepâncias apuradas (C2: €2.184,95 – 89,26%; C1: €2.402,57 – 23,65%) mantêm plena relevância jurídica.', 'The discrepancies found (C2: €2,184.95 – 89.26%; C1: €2,402.57 – 23.65%) maintain full legal relevance.') + '</p></div>';
	} else {
	...
	if(atf.outlierMonths.length > 0 && months.length > 0) {
		...
		outliersDiv.innerHTML = '<div style="color:#EF4444;font-weight:bold;font-size:0.8rem;margin-bottom:8px">' + _T('⚠ MESES COM OUTLIER (DESVIO > 2σ) — Indício qualificado Art. 104.º RGIT', '⚠ MONTHS WITH OUTLIER (DEVIATION > 2σ) — Qualified evidence Art. 104 RGIT') + '</div>' + '<div style="display:flex;flex-wrap:wrap;gap:8px">' + atf.outlierMonths.map(function(m) {
			var idx = months.indexOf(m);
			var lbl = m.length === 6 ? m.substring(0, 4) + '/' + m.substring(4) : m;
			var disc = atf.discrepancySeries[idx] || 0;
			return '<div data-disc-value="' + disc + '" style="background:rgba(239,68,68,0.2);border:1px solid rgba(239,68,68,0.5);border-radius:4px;padding:6px 12px;color:#FCA5A5;font-size:0.75rem">' + '<strong>' + escapeHtml(lbl) + '</strong><br>\u0394 ' + window.UNIFEDSystem.utils.formatCurrency(disc) + '</div>';
		}).join('') + '</div>';
	...
	legalDiv.innerHTML = '<strong style="color:rgba(0,229,255,0.7)">' + _T('Fundamentação Jurídica:', 'Legal Basis:') + '</strong> ' + 'O Art. 103.o e 104.o do RGIT distinguem o erro pontual da conduta dolosa mediante a demonstracao de iteracao. ' + 'O Score de Persistencia quantifica a sistematicidade das omissoes. ' + 'O Padrao Oportunistico (outliers em picos de faturacao) reforca o dolo especifico. ' + 'Art. 125.o CPP · ISO/IEC 27037:2012';
	...
	if(months.length > 0 && typeof Chart !== 'undefined') {
		try {
				...
				new Chart(cvs, {
					...
					data: {
						...
						datasets: [{
							...
							label: _T('Discrepância', 'Discrepancy'),
							...
							label: _T('Limiar 2σ', 'Threshold 2σ'),
					...
					options: {
						...
						plugins: {
							...
							tooltip: {
								...
								callbacks: {
									label: function(c2) {
										var _loc = (typeof window.currentLang !== 'undefined' && window.currentLang === 'en') ? 'en-GB' : 'pt-PT';
										return ' ' + c2.dataset.label + ': ' + new Intl.NumberFormat(_loc, {
											style: 'currency',
											currency: 'EUR'
										}).format(c2.raw || 0);
									}
			...
			console.warn('[UNIFED-ATF] Chart.js error:', cErr.message);
		}
	}
